...
router.post("/subtitles/chunk", expressJson({ limit: "128kb" }), (req, res) => {
  const { sessionId, chunkIndex, totalChunks, data } = req.body as Record<
    string,
    string | number
  >;
  if (!sessionId || chunkIndex === undefined || !totalChunks || !data) {
    res
      .status(400)
      .json({ error: "Missing sessionId, chunkIndex, totalChunks, or data." });
    return;
  }
  try {
...
router.post(
  "/subtitles/process-many",
  expressJson({ limit: "10kb" }),
  async (req, res) => {
    const apiKey =
    ...
    if (!apiKey) {
      res
        ...
        .json({
          error:
            "AssemblyAI API key not configured. Enter your key in the Subtitles page.",
        });
      return;
    }
    ...
    try {
      ...
      for (let idx = 0; idx < sessions.length; idx++) {
        const { sessionId, totalChunks, originalName, language } =
          sessions[idx];
        ...
        if (!uploadRes.ok) {
          throw new Error(
            `AssemblyAI upload failed (${uploadRes.status}): ${await uploadRes.text()}`,
          );
        }
        ...
        for (let attempt = 0; attempt < 6; attempt++) {
          if (attempt > 0) {
            ...
            logger.warn(
              { idx, originalName, attempt, waitMs },
        ...
        if (!txRes.ok) {
          throw new Error(
            `AssemblyAI transcript create failed (${txRes.status}): ${await txRes.text()}`,
          );
        }
        ...
        while (Date.now() < deadline) {
          ...
          if (pollData.status === "error") {
            throw new Error(
              `AssemblyAI error for "${originalName}": ${pollData.error ?? "unknown"}`,
            );
          }
          logger.info({ id, status: pollData.status }, "Polling...");
        }
        if (!completed)
          throw new Error(`Timeout waiting for "${originalName}"`);
        ...
        if (!srtRes.ok) {
          throw new Error(
            `SRT fetch failed (${srtRes.status}) for "${originalName}"`,
          );
        }
...
router.get("/subtitles/download/:filename", (req, res) => {
  ...
  const asName =
    typeof req.query["as"] === "string" ? req.query["as"] : filename;
...
router.get("/subtitles/download-page", (req, res) => {
  ...
  const urls: string[] = Array.isArray(rawU)
    ? (rawU as string[])
    : rawU
      ? [rawU as string]
      : [];
  ...
  const buttons = urls
    .map((u) => {
      const displayName = u.split("as=")[1]
        ? decodeURIComponent(u.split("as=")[1])
        : "file.srt";
