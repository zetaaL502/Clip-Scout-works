...
export function SubtitlePage() {
  ...
  const toggleCheck = (name: string) =>
    setEntries((prev) =>
      prev.map((e) =>
        e.file.name === name ? { ...e, checked: !e.checked } : e,
      ),
    );

  const setLang = (name: string, lang: string) =>
    setEntries((prev) =>
      prev.map((e) => (e.file.name === name ? { ...e, language: lang } : e)),
    );
    ...
    try {
      const res = await fetch("/api/subtitles/process-many", {
        ...
        headers: {
          ...
          "x-assemblyai-key": storage.getAssemblyAIKey(),
        },
  ...
  const qrPageUrl = (() => {
    ...
    results.forEach((r) =>
      params.append("u", `${window.location.origin}${r.downloadUrl}`),
    );
            ...
            <>
              ...
              >
                ...
                <p className="text-xs text-gray-500 mt-1">
                  MP3, WAV, MP4, M4A · max {MAX_FILES} files
                </p>
                {entries.length > 0 && (
                  <p className="text-xs text-[#22c55e] mt-2 font-medium">
                    {entries.length}/{MAX_FILES} added
                  </p>
                )}
              </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setLang(entry.file.name, e.target.value)
                            }
          ...
          {stage === "processing" && proc && (
            <>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    One file at a time · 12 s pause between each
                  </p>
                </div>
              </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500">
                          ...
                          {status === "queued" &&
                            "Uploaded — AssemblyAI transcribing…"}
              ...
              <p className="text-xs text-gray-600 text-center">
                Automatic 12 s pause between files · rate-limit errors
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-lg">
                    {results.length} SRT file{results.length !== 1 ? "s" : ""}{" "}
                    ready!
                  </p>
                  <p className="text-xs text-gray-500">
                    Download below or scan the QR to get all files on your phone
                  </p>
                </div>
              </div>
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-gray-500 text-center">
              Opens a download page with all {results.length} SRT file
              {results.length !== 1 ? "s" : ""}.<br />
